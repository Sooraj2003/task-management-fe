...
const Task = ({task}) => {
 ...
 const {_id,title,description,dueDate,priority,status}=task;
  ...
  try{  
    // Update on DB
    const res = await axios.patch(BASE_URL+"/task/update/"+_id,editedTask,{withCredentials:true})
    dispatch(addTask(res?.data?.task))
    
    // For now, just closing the edit mode
    setIsEditing(false);
   }catch(err){
  ...
  const handleDelete = async (_id) => {
   ...
   { 
    ...
    const res = await axios.delete(BASE_URL+"/task/delete/"+_id,{withCredentials:true})
  ...
  const handleComplete = async(_id) => {
    ...
    //Complete on DB
    const res = await axios.patch(BASE_URL+"/task/complete/"+_id,{},{withCredentials:true})
